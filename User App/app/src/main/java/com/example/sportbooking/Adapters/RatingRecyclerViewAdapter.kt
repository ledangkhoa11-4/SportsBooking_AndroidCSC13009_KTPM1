...
class RatingRecyclerViewAdapter(
    ...
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        var improveContent = ""
        if(maintitles[position].improvement!![0]) improveContent += "Service - "
        if(maintitles[position].improvement!![1]) improveContent += "Customer care - "
        if(maintitles[position].improvement!![2]) improveContent += "Field Information - "
        if(maintitles[position].improvement!![3]) improveContent += "Staff attitude - "
        if(maintitles[position].improvement!![4]) improveContent += "Surface quality - "
        if(maintitles[position].improvement!![5]) improveContent += "Field Facilities - "
        if(improveContent != "")
